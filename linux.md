...
## 用户类型
>[参考网址](https://www.linuxprobe.com/chapter-05.html)
*   UID
:   用户身份的唯一标识
    *   0
    :   管理员用户，root的UID为0
...
*   -rwx------ root(属主) root(属组)
    ...
    *   文件特殊权限
        *   SUID 4 `chmod u+s filename`
        :   文件权限所有者，临时的特殊权限授权方法(尚方宝剑) x->s,-->S
        *   SGID 2 `chmod -R g+s filename`
        :   文件所在组权限，让执行者临时拥有属组的权限,在某个目录中创建的文件自动继承该目录的用户组,x->s,-->S
        *   SBIT 1 `chmod -R o+t filename`
        :   其他用户权限，保护位,只允许文件所有者删除该文件,x->t,-->T
        
## 文件隐藏属性 
>+添加隐藏属性 -移除移除属性
*   chattr
:   设置文件的隐藏权限,`chattr +a demo.sh`
*   lsattr
:   显示文件的隐藏权限,`lsattr demo.sh`

## 文件访问控制列表
>管理文件的ACL规则
*   `setfacl -Rm u:freax:rwx /haha`
:   给haha目录的所有主添加rwx
*   `getfacl /haha`
:   显示文件上设置的ACL信息
